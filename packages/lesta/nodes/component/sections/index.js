...
export default async function(pc, specialty, nodeElement, proxies, create) {
  if (pc.sections) {
    const createSection = async (section, options) => {
    ...
    for await (const [section, options] of Object.entries(pc.sections)) {
      ...
      if (options.src) await createSection(section, options)
      sectionNode.createSection = async (v) => await createSection(section, v || options)
    }
  }
}